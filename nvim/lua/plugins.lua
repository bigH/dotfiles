...
function get_preview_setting()
    ...
    local preview_executable = os.getenv("DOT_FILES_DIR") ..
                                   '/auto-sized-fzf/auto-sized-fzf.sh'
...
n('<LEADER>e', function() fzf.files({cmd = 'fd --hidden'}) end)

n('<LEADER>E', function() fzf.files({cmd = 'fd --hidden --no-ignore'}) end)
...
require('lualine').setup({
    ...
    sections = {
        ...
        lualine_b = {'fileformat', {'filename', path = 1}, 'branch'},
        lualine_c = {},
        lualine_x = {},
...
require('bufferline').setup({options = {indicator = {style = 'underline'}}})
...
vim.api.nvim_set_hl(0, "RainbowDelimiterRed",
                    {fg = configs.colors.red, bg = configs.dark_bg_indent})
vim.api.nvim_set_hl(0, "RainbowDelimiterYellow",
                    {fg = configs.colors.yellow, bg = configs.light_bg_indent})
vim.api.nvim_set_hl(0, "RainbowDelimiterBlue",
                    {fg = configs.colors.blue, bg = configs.dark_bg_indent})
vim.api.nvim_set_hl(0, "RainbowDelimiterOrange",
                    {fg = configs.colors.orange, bg = configs.light_bg_indent})
vim.api.nvim_set_hl(0, "RainbowDelimiterGreen",
                    {fg = configs.colors.green, bg = configs.dark_bg_indent})
vim.api.nvim_set_hl(0, "RainbowDelimiterViolet",
                    {fg = configs.colors.violet, bg = configs.light_bg_indent})
...
local highlight = {
    "RainbowIndentRed", "RainbowIndentYellow", "RainbowIndentBlue",
    "RainbowIndentOrange", "RainbowIndentGreen", "RainbowIndentViolet"
}

local hooks = require("ibl.hooks")

hooks.register(hooks.type.HIGHLIGHT_SETUP, function()
    vim.api.nvim_set_hl(0, "RainbowIndentRed", {
        fg = configs.blendToBackground(configs.colors.red),
        bg = configs.dark_bg_indent
    })
    vim.api.nvim_set_hl(0, "RainbowIndentYellow", {
        fg = configs.blendToBackground(configs.colors.yellow),
        bg = configs.light_bg_indent
    })
    vim.api.nvim_set_hl(0, "RainbowIndentBlue", {
        fg = configs.blendToBackground(configs.colors.blue),
        bg = configs.dark_bg_indent
    })
    vim.api.nvim_set_hl(0, "RainbowIndentOrange", {
        fg = configs.blendToBackground(configs.colors.orange),
        bg = configs.light_bg_indent
    })
    vim.api.nvim_set_hl(0, "RainbowIndentGreen", {
        fg = configs.blendToBackground(configs.colors.green),
        bg = configs.dark_bg_indent
    })
    vim.api.nvim_set_hl(0, "RainbowIndentViolet", {
        fg = configs.blendToBackground(configs.colors.violet),
        bg = configs.light_bg_indent
    })
end)
...
if vim.env.OPENAI_API_KEY ~= nil then require("chatgpt").setup() end
...
n('<c-j>', function() sj.split({split = {recursive = true}}) end)

n('<c-k>', function() sj.join() end)

i('<c-j>', function() sj.split({split = {recursive = true}}) end)

i('<c-k>', function() sj.join() end)
