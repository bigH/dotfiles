...
function M.setup()
    ...
    local on_attach = function(_, bufnr)
        ...
        local function bn(a, b) n(a, b, {buffer = bufnr}) end
    ...
    local function configure_lsp_with_defaults(name, config)
        local total_config = vim.tbl_deep_extend('force', default_server_config,
                                                 config or {})
    ...
    cmp.setup({
        ...
        sources = cmp.config.sources({{name = 'emoji'}, {name = 'nvim_lsp'}}, {
    ...
    cmp.setup.cmdline(':', {
        sources = {
            {name = 'path'}, {name = 'cmdline'}, {name = 'cmdline_history'}
        }
    })
